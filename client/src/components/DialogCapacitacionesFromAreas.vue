  ...
  <main id="DialogCapacitacionesFromEstablecimientos">
    ...
    <v-dialog fullscreen v-model="show" @keydown.esc="closing()" hide-overlay transition="dialog-bottom-transition">
      ...
      <v-alert :value="true" color="error" icon="warning">
        No hay capacitaciones ingresadas en el sistema para consultar
      </v-alert>
    </template>
      </v-data-table>
    </v-layout>
  ...
  methods: {
    ...
    getPersonasFromEstablecimiento () {
      this.$store.dispatch('getAreaSola', this.areaId)
        .then((resp) => {
          console.log('Done1')
          let establecimientosId = this.$store.getters.areaSelected.establecimientosId
          console.log(establecimientosId)
          this.$store.dispatch('getPersonasFromEstablecimiento', establecimientosId)
            .then((resp) => {
              console.log('Done2')
